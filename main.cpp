...
#pragma region Texture読み込み
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> textureResource = CreateTextureResource(graphics.GetDevice(), metadata);
	Microsoft::WRL::ComPtr<ID3D12Resource> intermediasteResource = UploadTextureData(textureResource, mipImages, graphics.GetDevice(), cmdList_);
...
#pragma region リソース設定
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> materialResourceSprite = CreateBufferResource(graphics.GetDevice(), sizeof(Material));
	...
	Microsoft::WRL::ComPtr<ID3D12Resource> transformationMatrixResourceSprite = CreateBufferResource(graphics.GetDevice(), sizeof(TransformationMatrix));
	// データを書き込む
	...
	/*--Index用リソース作成--*/
	Microsoft::WRL::ComPtr<ID3D12Resource> indexResourceSprite = CreateBufferResource(graphics.GetDevice(), sizeof(uint32_t) * 6);
...
#pragma endregion
	...
	// Shaderをコンパイルする
	Microsoft::WRL::ComPtr<IDxcBlob> vs3DBlob = dxcCompiler.CompileShader(L"Object3D.VS.hlsl", L"vs_6_0");
	Microsoft::WRL::ComPtr<IDxcBlob> ps3DBlob = dxcCompiler.CompileShader(L"Object3D.PS.hlsl", L"ps_6_0");

	Microsoft::WRL::ComPtr<IDxcBlob> vs2DBlob = dxcCompiler.CompileShader(L"Object2D.VS.hlsl", L"vs_6_0");
	Microsoft::WRL::ComPtr<IDxcBlob> ps2DBlob = dxcCompiler.CompileShader(L"Object2D.PS.hlsl", L"ps_6_0");
